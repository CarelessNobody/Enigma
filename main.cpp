...
void init(set<pair<int, int>>& s, vector<string> &graph) { //Initialize set with locations to visit
...
int convertToIndex(pair<int, int> location, vector<string> &graph) { //Turns (x,y) into an index for 1D vector
  return (location.second * graph[0].length()) + location.first;
}

vector<pair<int,int>> dijkstra(vector<string> &graph, pair<int, int> start, pair<int, int> end) {
  if (graph[start.second][start.first] == '1' || graph[end.second][end.first] == '1')
    return {};

  //Set up
  pair location = start; //x, y
  ...
  while (predecessor[i] != pair<int,int>{-1,-1}) { //Only starting node will have this predecessor
...
float calcDistance(pair<int, int> start, pair<int, int> end) { //The heuristic
  return sqrt(pow(end.first - start.first, 2) + pow(end.second - start.second, 2)); //Just pythagorean theorem
}

vector<pair<int, int>> AStar(vector<string> &graph, pair<int, int> start, pair<int, int> end) {
  ...
  unordered_map<int, pair<int, int>> parent; //Key has to be an int since I will have to provide hash func otherwise
  ...
  while (!openSet.empty()) {
    ...
    for (auto p : openSet) {
      if (location == pair{-1,-1} ||
        calcDistance(p, end) + calcDistance(start, p) < calcDistance(location, end) + calcDistance(start, location))
    ...
    if (location == end) { //Found location, return path
...
void testDijkstra(vector<string> &graph, pair<int, int> start, pair<int, int> end) {
  vector<pair<int,int>> dPath = dijkstra(graph, start, end);
...
void testAStar(vector<string> &graph, pair<int, int> start, pair<int, int> end) {
  vector<pair<int,int>> aPath = AStar(graph, start, end );
...
int main() {
  vector<string> maze = getMaze("maze0_dim42.txt");
  ...
  testDijkstra(maze, {0,1} , {84,83} );
  cout << endl;
  testAStar(maze, {0,1} , {3,1});
